...
describe('Against NullOrEmpty Tests', () => {
  ...
  it('should return the input value if tests passes', () => {
    ...
    const checked2 = Guard.against().nullOrEmpty('Input');
    const checked3 = Guard.against().nullOrEmpty({ m: '10' });

    expect(checked).toBe(0);
    expect(checked2).toBe('Input');
    expect(checked3).toStrictEqual({ m: '10' });
  });
